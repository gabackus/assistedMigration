...
commSimulate <- function(n,P,X,y=1,years=100,extInit=F,extThresh=100,manage=NULL){
...
assistMigrate<-function(n,N,L,W,temp1d,t,AM){
  ...
  if(!isempty(SReloc)){
    ...
    locOpt <- sapply(SReloc, function(s) which.min(abs(zEst[s]-temp1d))+xLoc[s])
    
    # If locS is too big or too small, we need to fix that
    for(i in 1:SRL){
      s<-SReloc[i]
      if(locOpt[i]<(1+recRad[s])){
        locOpt[i] <- 1+recRad[s]
      }else if(locOpt[i]>(L-recRad[s]))
        locOpt[i] <- L-recRad[s]
    }
    
    # Identify the locations that receive inidividuals
    locOptAll <- t(sapply(1:SRL, function(i) (locOpt[i]-recRad[SReloc[i]]):(locOpt[i]+recRad[SReloc[i]])))
...
cSetup<-commSetup(S=S,L=L,W=W,compType="temp",tau=0,years=iYears+ccYears,tempYSD=tempYSD,tempYAC=tempYAC,tempLSD=tempLSD,QMean=QMean)
P<-cSetup$P
X0<-cSetup$X

n0 <- array(4,c(P$S,X0$L,X0$W))


cSim1<-commSimulate(n0,P,X0,years=iYears,extInit=F,extThresh=100)
...
W <- seq(65,320)
n1W <- n1[,W,1]
n1Persist <- which(rowSums(n1W)>0) # which species exist in interior region?
gMin <- min(P$gam[n1Persist]) # what is the shortest dispersal amongst these?
speciesF <- which(P$gam==gMin) # this is the target
# If you want to try other targets
# sMin <- min(P$sig[n1Persist]) # what is the narrowest thermal tolerance amongst these?
# speciesF <- which(P$sig==sMin) # this is the target
# nMin <- min(rowSums(n1[n1Persist,,1])) # what is the lowest population size amongst these?
# speciesF <- which(rowSums(n1[n1Persist,,1])==nMin) # this is the target

mSetup <- manageSetup(P,X1,years=100,AM=T,targs=speciesF,eta=42,rho=0.5)

cSimAM<-commSimulate(n1,P,X1,y=iYears+1,years=ccYears,manage=mSetup)

cSimNA<-commSimulate(n1,P,X1,y=iYears+1,years=ccYears)
plot(0:100,cSimAM$N[speciesF,],ylim=c(0,max(c(cSimAM$N[speciesF,],cSimNA$N[speciesF,]))))
lines(0:100,cSimNA$N[speciesF,])
